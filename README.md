...
1. **克隆仓库**
   ```bash
   git clone https://github.com/YOUR_USERNAME/clearview-ai.git
   cd clearview-ai
   ```
...
3. **配置 API Key**
   ...
   > ⚠️ **注意**: 视频生成功能需要付费的 Google Cloud Project
...
1. **API Key 安全**: 
   ...
   - 如果使用 GitHub，确保 `.env.local` 在 `.gitignore` 中
...
3. **环境支持**:
   - 支持本地开发和 AI Studio 环境
   - 本地运行时需要在 `.env.local` 中配置 API Key
